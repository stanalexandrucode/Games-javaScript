document.addEventListener('DOMContentLoaded', () => {
    ...
    //add onclick to each square in the grid
        squares[i].onclick = function(){
            //if the square below the current square is taken, can go on top of it
            if(squares[index + 7].classList.contains('taken')){
                ...
                //if the square below the current square is taken, can't go there
                } else {
                alert('cant go here');
                }
        }
    ...
    function checkBoard() {
        ...
        //check the 4 values in each winningArray
        for(let y = 0; y < winningArrays.length; y++) {
            ...
            //check to see if they all have the classname player two
            else if (square1.classList.contains('player-two') &&
                square2.classList.contains('player-two') &&
                square3.classList.contains('player-two') &&
                square4.classList.contains('player-two')) {
                //if they do, player-two is the winner as well as the chip positions
